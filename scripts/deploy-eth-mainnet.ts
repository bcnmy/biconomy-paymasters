...
import { ethMainnetConfigInfoProd } from "./configs";
...
const tokenConfig: TokenConfig = ethMainnetConfigInfoProd;
...
async function main() {
  ...
  for (const token of tokenConfig.tokens) {
    ...
    const derivedPriceFeedAddress = priceFeedAddress;
    console.log(
      `derived price feed address for token ${symbol} is ${derivedPriceFeedAddress}`
    );
