...
const StackupPaymaster = await ethers.getContractFactory("StackupVerifyingPaymaster");
const stackupPaymaster = await StackupPaymaster.deploy(entryPoint, verifyingSigner);

await stackupPaymaster.deployed();

console.log(
  `StackupPaymaster deployed at ${stackupPaymaster.address}`
);
}
